...
char Address = 0x00;
  ...
  I2C_ReStart();                    // Restart I2C
  I2C_Write((Device << 1) | 1);     // Initialize data read
  second = I2C_Read(ACK);           // Reads the register 'second'
  minute = I2C_Read(ACK);           // Reads the register 'minute'
  hour = I2C_Read(ACK);             // Reads the register 'hour'
  dow = I2C_Read(ACK);              // Reads the register 'dow'
  day = I2C_Read(ACK);              // Reads the register 'day'
  month = I2C_Read(ACK);            // Reads the register 'month'
  year = I2C_Read(NAK);             // Reads the register 'year'
  I2C_Stop();                
}
