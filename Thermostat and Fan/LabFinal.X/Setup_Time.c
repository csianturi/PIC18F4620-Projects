...
void Do_Setup_Time(void)
{
    ...
    while (In_Time_Setup == 1)                                                  //while still in the time setup screen
    {   
         if (check_for_button_input() == 1)                                     //checks if there is a button press
         {                                                                      //if the button pressed is between previous and EQ does good beep
                                                                                //if not bad beep
...
void Increase_Time()
{
            switch (Select_Field)                                               // Increments the proper field while setting the maximum and reset values
            {
                case 0:
                    setup_hour++;
...
void Decrease_Time()
{
            switch (Select_Field)                                               // Decrement the proper field and set the minimum and reset values
            {
                case 0:
...
void Go_Next_Field()                                                            //function used to move cursor forward
{
...
void Go_Prev_Field()                                                            //function used to move cursor backwards
{
...
void Do_Save_New_Time()                                                         //function used to save the new set time
{   
    DS3231_Write_Time();                                                        // Write time
    DS3231_Read_Time();                                                         // Read time
    Initialize_Screen();                                                        // Initialize the screen before returning  
    In_Time_Setup = 0;                                                          //leaves time setup
}
    
void Exit_Time_Setup()                                                          //function used to leave time setup screen without saving
{
    DS3231_Read_Time();                                                         // Read time
    Initialize_Screen();                                                        // Initialize the screen before returning  
    In_Time_Setup = 0;                                                          //leaves time setup screen
}
void Initialize_Setup_Time_Screen(void)                                         //initializes the time setup screen
{ 
...
void Update_Setup_Time_Screen(void)                                             //updates time setutp screen
{
...
void Update_Setup_Screen_Cursor_Forward(char field)                             //updates the cursor
{
    char xinc = 36;
    char yinc = 30;   
    switch (field)
...
void Update_Setup_Screen_Cursor_Backward(char field)                            //moves cursor backwards
{
    char xinc = 36;
    char yinc = 30;   
    switch (field)
