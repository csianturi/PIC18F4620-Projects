...
void Do_Setup_Heater_Temp(void)
{
    ...
    {
         if (check_for_button_input() == 1)                                     //checks if the button pressed is in between Play_Pause and EQ
        {                                                                       //if it is will do the button's corresponding action
             nec_ok = 0;                                                        //if another  button is pressed other than these, produce a bad beep
...
void Increase_Heater_Temp()                                                     //function used to increase the heater temp
{                                                                               //maximum amount the temp can be set is 120
	if(setup_heater_set_temp == 120) setup_heater_set_temp = 120;               //else will increment
    else setup_heater_set_temp++;

    	Update_Setup_Heater_Screen();                                           // Update screen with latest info

}

void Decrease_Heater_Temp()                                                     //function used to decrease the heater temp
{
	if(setup_heater_set_temp == 50) setup_heater_set_temp = 50;                 //minimum amount for the temp can be set is 50
    else setup_heater_set_temp--;                                               //else will decrement
...
void Exit_Setup_Heater_Temp()                                                   //function used to exit the setup screen without saving
{
...
void Do_Save_New_Heater_Temp()                                                  //function used leave the screen and save the heater temp
{
...
void Initialize_Setup_Heater_Screen(void)                                       //initializes the heater screen
{ 
...
void Update_Setup_Heater_Screen(void)                                           //updates the screen
{
    char dc_char1 = setup_heater_set_temp/100;                                  //the actual text for the temperature on the setup heater screen
    char dc_char2 = (setup_heater_set_temp%100) / 10;                           //each variable for the corresponding heater temp digit
