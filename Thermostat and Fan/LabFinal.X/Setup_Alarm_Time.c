...
void Do_Setup_Alarm_Time(void)
{
    ...
    {   
         ...
         {
             if (found >= Prev && found <= EQ)                                  //if the button pressed is in between previous and eq
             {                                                                  //will do a good beep and do different actions based on button pressed
                Do_Beep_Good();
             ...
             {
                 Do_Beep_Bad();                                                 //if not, will provide a bad beep
             }
        }                   
    }
}
...
{
            switch (Select_Alarm_Field)                                         //based on the field being edited, will increment if 
            {                                                                   //Plus is pressed, once a maximum value is achieved
                case 0:                                                         //will reset that field back to the minimum value
                    setup_alarm_hour++;
                    if (setup_alarm_hour == 24) setup_alarm_hour = 0;           // 24-->0
                    break;

                case 1:
                    setup_alarm_minute++;
                    if (setup_alarm_minute == 60) setup_alarm_minute = 0;       //60-->0
                    break;

                case 2:
                    setup_alarm_second++;
                    if (setup_alarm_second == 60) setup_alarm_second = 0;       //60-->0
                    break;
...
{       
            switch (Select_Alarm_Field)                                         //based on the field being edited, will decrement if
            {                                                                   //Minus is pressed, once a minimum value is achieved
                case 0:                                                         //will reset that field back to the maximum value
                    if (setup_alarm_hour == 0) setup_alarm_hour = 23;           // 0-->23
                    else --setup_alarm_hour;
                    break;

                case 1:
                    if (setup_alarm_minute == 0) setup_alarm_minute = 59;       //0-->59
                    else --setup_alarm_minute;
                    break;

                case 2:
                    if (setup_alarm_second == 0) setup_alarm_second = 59;       //0-->59
...
void Exit_Setup_Alarm_Time()                                                    //exits setup alarm time screen without saving
{
...
void Do_Save_New_Alarm_Time()                                                   //function used to save alarm time screen
{
...
void Update_Setup_Alarm_Time_Screen(void)
{
    ...
    setup_alarm_time[0]  = (setup_alarm_hour/10)  + '0';                        //the text for the setup alarm time screen
    setup_alarm_time[1]  = (setup_alarm_hour%10)  + '0';                        //for the corresponding hour/minute/second digit
